...
- [ ] **Светлая пара для `billy-aurora`.** В авто-режиме (`claude-green-light/claude-green`) он не участвует и остаётся тёмным. Нужен ли вообще — решать при следующем обращении к этой теме.
- [ ] **Сообщить апстриму два бага `pi-claude-code-ui`** (осветление chrome на светлых панелях; порча truecolor в `stripBackgroundAnsi`) — репа `pi-cc-tools`. До фикса живёт наш `patches/`.
...
- Авто-тема light/dark по системной теме macOS: светлая пара `claude-green-light` + разбор трёх причин, почему тема не переключалась (см. `light-theme.md`).
